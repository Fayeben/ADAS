...
class LinearHelixTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, kqv_bias=False, act_layer=nn.GELU, norm_layer=nn.LayerNorm, mlp_ratio=1):
    ...
    def forward(self, x1, x2):
        ...
        res1 = x1
        res2 = x2
        x1 = self.linear1(x1["feats"]).reshape(1, 1, A, 16)
        x2 = x2["feats"].reshape(1, 1, A, 16)
        B, C, H, W = x1.shape
        ...
        out1 = out1.transpose(-2, -1).contiguous().squeeze(0).reshape(A, 16)
        out2 = out2.transpose(-2, -1).contiguous().squeeze(0).reshape(A, 16)
        res1['feats'] = out1
        res2['feats'] = out2
        return res1, res2