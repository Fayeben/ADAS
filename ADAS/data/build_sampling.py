...
from ADAS.data.collate_sampling import get_collate_scn
from ADAS.data.collate_sampling_audi import get_collate_scn_audi
from ADAS.common.utils.sampler import IterationBasedBatchSampler
from ADAS.data.nuscenes.nuscenes_dataloader_sampling import NuScenesSCN_sampling
from ADAS.data.nuscenes.nuscenes_dataloader_sampling import NuScenesSCN_sampling
from ADAS.data.a2d2.a2d2_dataloader import A2D2SCN
from ADAS.data.semantic_kitti.semantic_kitti_dataloader_sampling import SemanticKITTISCN_sampling
...
def build_dataloader(cfg, mode='train', domain='source', start_iteration=0, halve_batch_size=False):
    ...
    if dataset_cfg.TYPE == 'NuScenesSCN':
        if mode == 'train':
            dataset = NuScenesSCN_sampling(split=split,
        ...
        dataset = SemanticKITTISCN_sampling(split=split,
                                   output_orig=not is_train,
                                   **dataset_kwargs,
                                   **augmentation)
    else:
    ...
    if 'SCN' in dataset_cfg.TYPE:
        if dataset_cfg.TYPE == 'A2D2SCN' or dataset_cfg.TYPE == 'SemanticKITTISCN':
            collate_fn = get_collate_scn_audi(is_train)
        else:
