...
class A2D2Base(Dataset):
    ...
    def __init__(self,
        ...
        with open("/mnt/lustre/feiben/xmuda/xmuda/cams_lidars.json", 'r') as f:
            self.config = json.load(f)
        # self.config = json.load(io.BytesIO(client.get('s3:/feiben/audi/cams_lidars.json'))
        ...
        for curr_split in split:
            ...
            print(self.preprocess_dir, curr_split)
            self.data.extend(read_s3_pkl("feiben", osp.join(self.preprocess_dir, curr_split + '.pkl')))
        # print("++++", self.data[0]["camera_path"])
        with open("/mnt/lustre/feiben/xmuda/xmuda/class_list.json", 'r') as f:
...
class A2D2SCN(A2D2Base):
    ...
    def __getitem__(self, index):
        ...
        if self.use_image:
            ...
            img_path = osp.join("s3://feiben", self.preprocess_dir, data_dict['camera_path'])

            # image = Image.open(img_path)
            # img_path = osp.join(self.nuscenes_dir, data_dict['camera_path'])
            # sdk_local_bytes = self.client.get(img_path, update_cache=True)
            # imgs = np.frombuffer(sdk_local_bytes, dtype=np.float32).reshape(-1, 4).copy()
            # image = Image.from_numpy(imgs)
