...
from ADAS.common.solver.build import build_optimizer, build_scheduler
from ADAS.common.utils.checkpoint import CheckpointerV2
from ADAS.common.utils.logger import setup_logger
...
from ADAS.discriminator import BEVDiscriminator_Conv_2
...
def train(cfg, output_dir='', run_name=''):
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        # activate sample
        # ---------------------------------------------------------------------------- #
        source_list = get_dataset_list(osp.join(cfg.DATASET_SOURCE.NuScenesSCN.preprocess_dir, cfg.DATASET_SOURCE.TRAIN[0] + '.pkl'), oss=True)

        if iteration % 1000 == 0 and iteration != start_iteration:
            sample_save_path = os.path.join(cfg.output_dir,'save_pkl_2d_3d_attn_source')
            frame_score = active_evaluate(Attention, model_2d, model_3d, d_main, train_dataloader_src)
...
def main():
    ...
    sys.path.append("../")
    from ADAS.common.config import purge_cfg
    from ADAS.config.xmuda import cfg
